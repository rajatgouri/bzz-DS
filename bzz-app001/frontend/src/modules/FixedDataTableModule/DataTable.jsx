...
import { CloseCircleTwoTone, EyeFilled } from "@ant-design/icons";
...
export default function DataTable({ config }) {
  ...
  let { entity, dataTableColumns, getItems, reload,  getFilterValue, dataTableTitle , userList, showFooter = true, openingModal, confirmModal, AddIcon, deleteIcon = false, openPdf, isLoading, refresh, openSortModal} = config;
  ...
  const handleAddressedValue  = async (id, value) => {
   
    setLoading(true)
    await request.update(entity, id, {Validate: value})
  ...
  const newDataTableColumns = dataTableColumns.map((obj) => {
    ...
    if (obj.dataIndex == "ManagementAccess"  || obj.dataIndex == "AdminAccess"  ) {
      return ({
        ...obj,
        render: (text, row) => {
        ...
        render: (text, row) => {
          ...
          return {
            ...
            children: (
                ...
                : 
                <div >
                <Select value={text}  style={{width: "100%"}} onChange={(e) => handleAddressedValue( row.ID, e)}>
      ...
      return ({
        ...
        render: (text, row) => {
          return {
            ...
            children: (
               <EyeFilled onClick={() => openPdf(row.FileName, row.Patient  , row['Parent'] ? 1 : row.Page)}/>
      ...
      return ({
        ...
        render: (text, row) => {
          ...
          return {
            ...
            children: (
              <span style={{color: text== 'No' ? 'red': 'green'}}>{text}</span>
              )
          };
        },
      })
    ...
    <div className= {entity == 'epic-productivity1' ? "wq-fixed-table epic-productivity1" : "wq-fixed-table"}>
          <Row gutter={[24,24]}>
...

            {
              ...
              {
               deleteIcon ? 
                <Button  style={{marginLeft: "10px"}} onClick={() => deleteTable()}>
                  <DeleteOutlined/>
                </Button>
                :  null
              }
              ...
              {
               refresh ? 
                <Button  style={{marginLeft: "10px"}} onClick={() => loadTable()}>
                  <ReloadOutlined/>
                </Button>
                :  null
              }
