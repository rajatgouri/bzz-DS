...
export default function HBDashboard() {
  ...
  const load = () => {
    (async () => {
      ...
      const [ wq5508Progress, wq1075Progress , feedbackProgress, ] = await Promise.all([ request.list("wq5508progress"), request.list("wq1075progress"), request.list("feedback"), ]);
      ...
      <Row gutter={[20, 20]}>
        
       {
          cardData.map(( data) => {
            ...
            percent1={data?.wq5508?.ChargesToReview}
            percent2={data?.wq1075?.ChargesToReview}
      ...
      <Col className="gutter-row" style={{width: "20%"}}>
          <div className="whiteBox shadow" style={{ height: "100%" }}>
            ...
            >
              ...
              <div style={{marginTop: "-10px", textAlign: "left"}} dangerouslySetInnerHTML={{ __html: reminders }}>
              </div>  
