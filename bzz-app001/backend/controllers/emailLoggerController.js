...
const FullModel = "[COHTEAMS].[dbo].[COH_MasterStaff_PSoft]";
...
endpoints.search = async (req, res) => {
  try {
    ...
    var query = `select [Start Timestamp], [Duration in Seconds] from ${Model}  Left JOIN ${FullModel} ON ${Model}.Email = ${FullModel}.EMAIL_ADDR  where FIRST_NAME = '${filter['FIRST_NAME']}' and [Finish Timestamp] > '${filter['StartTime'][0]}' and  [Finish Timestamp] < '${filter['StartTime'][1]}'`;
...
endpoints.list1 = async (req, res,) => {
  try {
    ...
    let {EMPID, date} =  JSON.parse(req.query.filter)
      ...
      const query = `select * from ${Model}  Left JOIN ${FullModel} ON ${Model}.Email = ${FullModel}.EMAIL_ADDR where EMPID = ${EMPID} and [Finish Timestamp] < '${date}' and  [Finish Timestamp] > '${date1}' ORDER BY [Finish Timestamp] DESC`;
      const { recordset: result } = await sql.query(query);
      ...
      if (filter[key]) {
        ...
        switch (key) {
          case "FIRST_NAME": {
            ...
            filterQuery += +filter[key] !== null ?   "[COHTEAMS].[dbo].[COH_MasterStaff_PSoft].[FIRST_NAME] IN (" + valueQuery + ") and " : "";
            break
          }
