...
const Model = "RAC";
const ColumnModel = "RACColumns"


setInterval(() => {
    UpdateDaysTillDueDate()
}, 50000)
...
endpoints.exports = async (req, res) => {
    try {
        ...
        let file = `SmartApp_RAC_${date}.xlsx`
...
const InsertItem = async (values) => {
    ...
    try {
        return new Promise(async (resolve, reject) => {
            ...
            await sql.query(`Update RAC set [Number of Claims] = (
                SELECT COUNT(*) from RAC where [Internal Number] = ${values['Internal Number']} and ([Archive] = '0' or [Archive] IS NULL)
            ) where [Internal Number] = ${values['Internal Number']}`)
            ...
            if (result.length > 0) {
                ...
                if ((+values['Number of Claims'] - result.length) > 0) {
                    ...
                    for (let i = 0; i < counter; i++) {
                        ...
                        delete values['HMS Audit ID']
                        delete values['RAC Case Number']
                        delete values['Patient Name']
                        delete values['MRN']
                        delete values['Claim Number']
                        delete values['Acct Number']
                        delete values['DOS From']
                        delete values['DOS To']
                        delete values['CPT Charge Amount']
                        delete values['CPT/J/HCPCS']
                        delete values['Drug/Procedure']
    ...
    try {
        ...
        if (result.success) {
            return res.status(200).json({
                success: true,
                result: {},
                message: `RAC added successfully!`
            });
        } else {
            return res.status(200).json({
                success: false,
...
const logger = async (values) => {
    ...
    const insertQuery = `insert into RAC_Logger ${columnsQ} values ${valuesQuery}`
    ...
    try {
       ...
       if (response.success) {
        ...
        if (all == 'Yes') {
            ...
            v['RAC Due Date'] = values['RAC Due Date']
            ...
            let vQuery = ''
            for (key in v) {
                if (v[key] == null ||v[key] == undefined || v[key] == 'null' || v[key] == 'undefined') {
...
endpoints.filters = async (req, res) => {
    try {
        ...
        const [{ recordset: Patient }, { recordset: Status }, { recordset: DrugProcedure }, { recordset: User }, { recordset: OpenClosed },  { recordset: InternalNumber }, {recordset: RecoupmentRationale}] = await Promise.all([
            (
                filter ? 
            ...
            (
                filter ?
                await sql.query(`Select DISTINCT([Drug/Procedure]) from ${Model} where [Open/Closed] IN (${filter.map((f) => "'" + f + "'")})`)
                :
                await sql.query(`Select DISTINCT([Drug/Procedure]) from ${Model} where [Open/Closed] IS NULL`)
            ),
            (
                filter ?
