...
const endpoints = methods.crudController("CERT");
...
endpoints.update = async ( req, res) => {
    try {
      ...
      if(values.Status == 'Compliance Review Complete Okay to SEND') {
        ...
        const {recordset: result} = await sql.query(`SELECT [Internal Tracking No.] from ${Model} where ID = ${id}`)
        ...
        mailer(
            ...
            `
            <p>
                An update occurred in CERT- compliance review was completed and it’s okay to send.
                Please click on the link below to look up the necessary records. <br>

                Internal Tracking No. : ${result[0]['Internal Tracking No.']} <br><br>
                https://10.30.142.17:8004/cert
            </p>
            `
        )
...
endpoints.list = async (req, res,) => {
    try {
        ...
        //             case "Date CERT Submitted to CMS": {
...
endpoints.exports = async (req, res) => {
    try {
        ...
        worksheet['!autofilter']={ref:"A1:AB1"};
        ...
        let file = `SmartApp_CERT_${date}.xlsx`
...
endpoints.filters = async (req, res) => {
    try {
        ...
        const [{ recordset: Patient }, { recordset: Status },{ recordset: ServiceType },{ recordset: User },{ recordset: OpenClosed }, { recordset: InternalTrackingNo },  {recordset: RecoupmentRationale}] = await Promise.all([
            (
                filter ?
            ...
            (
                filter?
                await sql.query(`Select DISTINCT ([Service Type])  as Status from ${Model}  where [Open/Closed] IN (${filter.map((f) => "'" + f+ "'")})`)
                :
            await sql.query(`Select DISTINCT ([Service Type])  as Status from ${Model}  where [Open/Closed] IS NULL`)
            ...
            (
                filter ? 
                await sql.query(`Select DISTINCT([Internal Tracking No.]) from ${Model} where [Open/Closed] IN (${filter.map((f) => "'" + f + "'")})`)
                :
                await sql.query(`Select DISTINCT([Internal Tracking No.]) from ${Model} where [Open/Closed] IS NULL`)
            )  ,
            (
                filter ? 
